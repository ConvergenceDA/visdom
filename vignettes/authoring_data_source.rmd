...
Returns a date.frame with variables (columns) as below:
...
The above description is for hourly meter data, though could do analogously for 15-minute data.  In that case, perhaps interval names like `Q1` to `Q96` (Q for quarterly).
...
Returns a data.frame for the weather in the specified zip code, where each row is an instance of weather variables (e.g., each hour).  Columns are:

     Name          |    Type               |     Example                  |     Description
-------------------|-----------------------|------------------------------|--------------------------------------
    `dates`        |  "POSIXct" "POSIXt"   |   `2013-01-01 01:00:00 PST`  |   Time instance   
    `temperaturef` |  `numeric`            |   65                         |   Temperature in degrees F at given time  
    `pressure`     |  `numeric`            |   30                         |   inches Hg (mercury) 
    `hourlyprecip` |  `numeric`            |  12.19631                  |   
    `dewpointf`    |    |     |              


dates (required of type POSIXct, with time at whatever intervals observations are available in, ideally with hourly or less intervals), temperaturef (required), pressure, hourlyprecip, dewpoint.

QUESTIONS:
-Pressure: inches Mercury?  If so should be closer to 30, where example is 19
-Hourly Precip: values here are indicator 0 or 1, whereas in my weather data they are postive real values that can go above 1?  I summed a few yearly for zip code and they turned out in the 15-20 range.
