...
Next you must provide implementation of your feature extraction algorithms. First, we wrap the return values of the VISDOM internal function `basicFeatures()` with a named list, the required output format of any feature function. Basic features include max/min/mean, range, variance, hour-of-day, peak timing, and other basic statistical extracts from the passed meter data `meterData`. Note that it relies on the presence of weather data in the meter data object.

```{r}
basicFeaturesfn = function(meterData,ctx,...) {
  return( as.list(basicFeatures(meterData)) )
}
```
...
ctx$start.date = as.Date('2010-10-15')
ctx$end.date   = as.Date('2011-10-15')
```
...
Once your features are computed, you may logically want to save or export them forIn `util-export.R`, there are several functions that are designed to support exports of your feature data to various useful formats.
...
exportData(runPlusCensusDF,name='myFeatures',format='csv') # csv extension automatically added
