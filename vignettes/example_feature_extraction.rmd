...
```{r eval=F}
library(RMySQL)
exportData(df = as.data.frame(runDF), 
           name='visdom_example', 
           format='database', 
           conn=conf.dbCon(dbCfg('visdom_features_db.cfg')), 
           overwrite=TRUE ) # the rows in runDF will be found in the table

exportData(df = as.data.frame(runDF), 
           name='visdom_example', 
           format='database', 
           conn=conf.dbCon(dbCfg('visdom_features_db.cfg')), 
           overwrite=FALSE ) # a second copy of the rows in runDF are appended to the same table
```
